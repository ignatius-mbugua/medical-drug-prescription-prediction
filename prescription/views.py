...
# symptoms create
@login_required
def symptoms_create(request):
    return render(request, 'prescription/symptoms-create.html')

#prescribe medicine
@login_required
def prescribe_medicine(request):
    # TODO: predict medicine
    if request.POST:
        diarrhoea = request.POST['diarrhoea']
        nausea_and_vomiting = request.POST['nausea_and_vomiting']
        dehydration = request.POST['dehydration']
        muscle_cramps = request.POST['muscle_cramps']
        shock = request.POST['shock']
        rash = request.POST['rash']
        fever = request.POST['fever']
        headache = request.POST['headache']
        body_aches = request.POST['body_aches']
        breathing_difficulties = request.POST['breathing_difficulties']
        blood_in_waste = request.POST['blood_in_waste']

        path_to_model = os.getcwd() + '/prescription/static/prescription/models/md_waterborne'
        with open(path_to_model, 'rb') as f:
            model = pickle.load(f)

        prediction = model.predict([[diarrhoea, nausea_and_vomiting, dehydration, muscle_cramps, shock, rash, fever, headache,
        body_aches, breathing_difficulties, blood_in_waste]])
        ...
        if disease == "Cholera":
            ...
            dosage = 3
        elif disease == "Bilharzia":
            medicine = "Biltricide"
            dosage = 4

        prescription = Prescription(user=request.user, disease_identified=disease, medicine=medicine)
        prescription.save()
...
def prescriptions_history(request, id):
    prescriptions = Prescription.objects.filter(user=id).order_by('-created_at')
    context = {
        'prescriptions': prescriptions
    }
    return render(request, 'prescription/prescriptions-history.html', context)
...
@login_required
def prescribe_medicin(request, id):
